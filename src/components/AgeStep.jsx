            ...
            {stepData.options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`group relative flex flex-col items-center p-3 rounded-[16px] border-2 transition-all duration-200 bg-white cursor-pointer overflow-hidden ${
                    isSelected 
                ...
                >
                  ...
                  <div className={`absolute top-2 right-2 z-20 w-5 h-5 rounded-full flex items-center justify-center transition-all ${
                    isSelected ? 'bg-[#FF3D7F] text-white' : 'bg-[#FFF9F6] border border-[#DFC9D3]'
                  }`}>
                    {isSelected && <Check className="w-3.5 h-3.5 stroke-[3]" />}
                  </div>

                  {/* FULL-FRAME IMAGE DISPLAY */}
                  <div className="w-full h-36 sm:h-44 mb-2 rounded-xl overflow-hidden bg-[#FFF9F6] flex items-center justify-center relative border border-[#F0E3E9]">
                    <img 
                      src={opt.image} 
                      alt={opt.label}
                  ...
                  <span className="text-xs sm:text-sm font-bold text-center leading-tight py-0.5 font-heading">
                    {opt.label}
                  </span>
                </button>
              );
