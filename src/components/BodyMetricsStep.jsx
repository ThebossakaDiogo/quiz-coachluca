...
import { Sparkles, ShieldCheck, ArrowLeft, Ruler, Weight, Activity, CheckCircle2, ArrowRight, Minus, Plus, Zap, Bookmark } from 'lucide-react';
    ...
    <div className="relative min-h-dvh overflow-hidden bg-gradient-to-b from-[#F6DBEE] via-[#F9EDF6] to-[#FCF5FA] py-5 px-3.5 sm:px-5 flex flex-col justify-center items-center font-body text-[#1F121C]">
      ...
      <div className="relative z-10 w-full max-w-lg mx-auto space-y-3.5">
        ...
        <div className="relative rounded-[26px] p-6 text-center text-white shadow-[0_16px_36px_-8px_rgba(216,56,125,0.35)] quiz-question-banner overflow-hidden animate-pop">
          ...
          <p className="text-xs sm:text-sm text-white/90 leading-relaxed max-w-md mx-auto font-medium pt-1 font-body">
            Ajusta los medidores interactivos para calcular tu IMC y fuerza neuromuscular.
          </p>
        </div>

        {/* MAIN WHITE CARD CONTAINER */}
        <div className="bg-white rounded-[26px] p-5 sm:p-6 shadow-xl border border-[#F0DCEB] space-y-4 quiz-card animate-pop">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left pt-1">
            ...
            {/* HEIGHT CONTROL */}
            <div className="bg-[#FDF4FA] rounded-[20px] p-4 border border-[#F0DCEB] space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-[#4A154B] uppercase tracking-wider flex items-center gap-2 font-heading">
                  <Ruler className="w-4 h-4 text-[#E63988]" />
                  Estatura
                </span>
                <span className="text-xl font-black text-[#1F121C] font-heading tracking-tight bg-white px-3.5 py-0.5 rounded-xl border border-[#F0DCEB] shadow-2xs">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="w-10 h-10 rounded-xl bg-white border-2 border-[#F0DCEB] hover:border-[#E63988] active:scale-95 text-[#4A154B] font-black text-xl flex items-center justify-center transition-all shadow-2xs shrink-0 cursor-pointer"
                >
                ...
                <div className="flex-1 space-y-1">
                  <input
                    ...
                    className="w-full h-2.5 bg-[#F0DCEB] rounded-lg appearance-none cursor-pointer accent-[#E63988]"
                  />
                ...
                <button
                  type="button"
                  onClick={() => handleHeightStep(1)}
                  className="w-10 h-10 rounded-xl bg-[#E63988] text-white hover:bg-[#B81E64] active:scale-95 font-black text-xl flex items-center justify-center transition-all shadow-md shrink-0 cursor-pointer"
                >
                ...
                {[155, 160, 165, 170, 175].map((val) => (
                  <button
                    ...
                    className={`flex-1 py-1 text-xs font-bold rounded-xl border transition-all cursor-pointer font-heading ${
                      height === val
                        ? 'bg-[#E63988] text-white border-[#E63988] shadow-xs'
                        : 'bg-white text-[#635360] border-[#F0DCEB] hover:border-[#E63988]/40'
                    }`}
                  >
                    {val}
                  </button>
            ...
            <div className="bg-[#FDF4FA] rounded-[20px] p-4 border border-[#F0DCEB] space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-[#4A154B] uppercase tracking-wider flex items-center gap-2 font-heading">
                  <Weight className="w-4 h-4 text-[#E63988]" />
                  Peso Actual
                </span>
                <span className="text-xl font-black text-[#1F121C] font-heading tracking-tight bg-white px-3.5 py-0.5 rounded-xl border border-[#F0DCEB] shadow-2xs">
            ...
            <div className={`p-4 rounded-[20px] border ${bmiDetails.bg} space-y-2.5 shadow-sm transition-all duration-300`}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-black uppercase tracking-wider text-[#1F121C] flex items-center gap-1.5 font-heading">
                  <Activity className="w-4 h-4 text-[#E63988] animate-pulse" />
                  IMC Biométrico:
                </span>
                <span className="text-lg font-black text-[#1F121C] bg-white px-3 py-0.5 rounded-xl border border-[#F0DCEB] font-heading shadow-2xs">
              ...
              <div className="space-y-1">
                <div className="h-2.5 w-full bg-[#F0DCEB] rounded-full overflow-hidden p-0.5 relative">
                  <div 
                    className="h-full bg-[#E63988] rounded-full transition-all duration-500"
          ...
          {/* TRUST BADGE */}
          <div className="pt-2 text-center border-t border-[#F0DCEB] flex items-center justify-center gap-1.5 text-xs font-medium text-[#968493]">
